...
model.compile(loss='categorical_crossentropy', metrics=['accuracy','mse','mae','mape'], optimizer='adam')
model.summary()
...
history = model.fit(X_train,
    ...
    callbacks = [
                tensorflow.keras.callbacks.ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
                tensorflow.keras.callbacks.ReduceLROnPlateau(monitor='val_loss',factor=0.5,verbose=1,patince=5,min_lr=0.000001),
                tensorflow.keras.callbacks.EarlyStopping(monitor='val_loss', patience=50, verbose=1, mode='auto')
